export const images = {
    logo: require("./irhereLogo.png"),
    auFlag: require("./flag_au.png"),
    quarantineIcon: require("./quarantine.png"),
    generalIcon: require("./general.png"),
    whereTo: require("./whereTo.png"),
    welcomImage: require("./welcomImage.png"),
    leftArrow: require("./leftArrow.png"),
    settingIcon: require("./setting.png"),
    helpIcon: require("./help-circle.png"),
    homeIcon: require("./home.png"),
    historyIcon: require("./list.png"),
    notificationIcon: require("./bell.png"),
    locationIcon: require("./locationIcon.png"),
    rightArrowIcon: require("./rightArrowIcon.png"),
    greenCheckIcon: require("./greenCheckIcon.png"),
    optionIcon: require("./option.png"),
    noNotificationIcon: require("./noNotificationIcon.png"),
    contactIcon: require("./users.png"),
    requestLocationIcon: require("./requestLocationIcon.png"),
    currentLocation: require("./currentLocation.png"),
    cameraIcon: require("./cameraIcon.png"),
    accountIcon: require("./accountIcon.png"),
    logoutIcon: require("./logoutIcon.png"),
    leftArrowIcon: require("./leftArrowIcon.png"),
    crossIcon: require("./crossIcon.png"),
    selfImage: require("./selfImage.jpeg"),
    tickIcon: require("./check.png"),
    rightFullArrow: require("./right-arrow.png"),
    unboldIcon: require("./left.png"),
    location: require("./location.png"),
    cameraImage: require("./camera.png"),
    hidePasswordIcon: require("./hidePasswordIcon.png"),
    unhidePasswordIcon: require("./Unhide.png"),
    bottomArrowIcon: require("./bottomArrow.png"),
    tickBlueIcon: require("./tickBlueIcon.png"),
    scannerIcon: require("./Scanner2.png"),
    quarantineDayIcon: require("./quartineDay.png"),
    calendarIcon: require("./calendar.png"),
    homeLocationIcon: require("./homeLocation.png"),
    bellIcon: require("./bellIcon.png"),
    ic_launcher: require("./ic_launcher.png"),
    circularTickIcon: require("./circularTick.png"),
    certificateItem: require("./certificateItem.png"),
    linkIcon: require("./link.png"),
    notificationIcon:require("./notificationIcon.png"),
    requestSomeoneLocation:require("./requestSomeoneLocation.png"),
    trustedContacts:require("./trustedContacts.png"),
    verfiyLocation:require("./verfiyLocation.png"),
    labelIcon:require("./labelIcon.png"),
    option:require("./option.png"),
    imagePlaceholder:require("./imagePlaceholder.png"),
    unblockLocation:require("./unablelocation.png"),
    unableCamera:require("./unableCamera.png"),
    searchIcon:require("./searchIcon.png"),
    contactScreen:require("./contactScreenMenue.png"),
    editAddress:require("./editAddress.png"),
    removeAddress:require("./removeAddress.png"),
    cameraPermission:require("./cameraPermission.png"),
    locationPermission:require("./locationPermission.png"),
    permissionAllow:require("./permissionAllow.png"),
    contactPermission:require("./contactPermission.png"),
    recycleBinIcon:require("./recycleBinIcon.png"),
    redCross:require("./redCross.png"),
    contactPermissionS:require("./contactPermissionS.png"),
}